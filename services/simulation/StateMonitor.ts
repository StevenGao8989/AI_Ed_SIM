...
export interface AnomalyDetection {
  type: 'divergence' | 'oscillation' | 'instability' | 'energy_leak';
...
// 状态分析报告
export interface StateAnalysisReport {
  timestamp: number;
  totalSteps: number;
...
export class StateMonitor {
  ...
  private currentState: SimulationState | null;
   ...
   * 计算性能指标
   */
  private calculatePerformanceMetrics(state: SimulationState, stepStartTime: number): PerformanceMetrics {
    const currentTime = Date.now();
   ...
   * 计算稳定性分数
   */
  private calculateStabilityScore(state: SimulationState): number {
    // 简化的稳定性计算
    let totalVelocity = 0;
    let objectCount = 0;
   ...
   * 检测异常
   */
  private detectAnomalies(state: SimulationState): AnomalyDetection | null {
    // 检测发散
    ...
    if (energyLeak) return energyLeak;
   ...
   * 检测发散
   */
  private detectDivergence(state: SimulationState): AnomalyDetection | null {
    // 简化的发散检测
    for (const obj of Object.values(state.objects)) {
      const speed = Math.sqrt(
        Math.pow(obj.velocity.x, 2) + 
   ...
   * 检测振荡
   */
  private detectOscillation(state: SimulationState): AnomalyDetection | null {
    // 简化的振荡检测
   ...
   * 检测不稳定性
   */
  private detectInstability(state: SimulationState): AnomalyDetection | null {
    // 简化的不稳定性检测
    for (const obj of Object.values(state.objects)) {
   ...
   * 检测能量泄漏
   */
  private detectEnergyLeak(state: SimulationState): AnomalyDetection | null {
    // 简化的能量泄漏检测
   ...
   * 计算总能量
   */
  private calculateTotalEnergy(state: SimulationState): number {
    let totalEnergy = 0;
   ...
   * 计算总动量
   */
  private calculateTotalMomentum(state: SimulationState): number {
    let totalMomentum = 0;
   ...
   * 获取最新报告
   */
  getLatestReport(): StateAnalysisReport {
   ...
   * 生成分析报告
   */
  private generateReport(): StateAnalysisReport {
    const currentTime = Date.now();
   ...
   * 更新配置
   */
  updateConfig(config: Partial<StateMonitorConfig>): void {
