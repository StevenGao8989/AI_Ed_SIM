...
import { 
  PhysicsIR, IRParameter, IRModule, IRConstraint, IREquation, IRPhysicalQuantity,
  IRConservationLaw, DimensionCalculator, PHYSICS_CONSTANTS, IRDimension 
} from './PhysicsIR';
...
export interface ValidationOptions {
  strict: boolean;           // 严格模式
  check_physics: boolean;    // 检查物理逻辑
  check_units: boolean;      // 检查单位一致性
  check_dependencies: boolean; // 检查依赖关系
  check_equations: boolean;  // 检查数学方程
...
export class IRValidator {
   ...
   * 主验证方法
   */
  validate(ir: PhysicsIR, options: Partial<ValidationOptions> = {}): IRValidationResult {
    ...
    try {
      // 1. 结构验证
      if (opts.verbose) console.log('🔍 开始结构验证...');
      ...
      // 2. 逻辑验证
      if (opts.verbose) console.log('🔍 开始逻辑验证...');
      ...
      if (opts.check_dependencies) {
        if (opts.verbose) console.log('🔍 开始依赖关系验证...');
      ...
      if (opts.check_physics) {
        if (opts.verbose) console.log('🔍 开始物理逻辑验证...');
      ...
      // 5. 单位一致性验证
      if (opts.check_units) {
        if (opts.verbose) console.log('🔍 开始单位一致性验证...');
      ...
      if (opts.check_equations) {
        if (opts.verbose) console.log('🔍 开始数学方程验证...');
   ...
   * 结构验证
   */
  private validateStructure(ir: PhysicsIR, options: ValidationOptions): { valid: boolean; errors: string[]; warnings: string[] } {
    const errors: string[] = [];
    const warnings: string[] = [];
    ...
    if (!ir.metadata) {
      ...
      if (!ir.metadata.system_type) errors.push('缺少系统类型');
      if (!ir.metadata.version) warnings.push('缺少版本信息');
    }
    ...
    if (!ir.simulation) {
      ...
      if (!ir.simulation.duration) errors.push('缺少仿真时长');
      if (!ir.simulation.time_step) errors.push('缺少时间步长');
      if (!ir.simulation.solver) errors.push('缺少求解器');
    }
   ...
   * 逻辑验证
   */
  private validateLogic(ir: PhysicsIR, options: ValidationOptions): { valid: boolean; errors: string[]; warnings: string[] } {
    const errors: string[] = [];
    const warnings: string[] = [];
   ...
   * 依赖关系验证
   */
  private validateDependencies(ir: PhysicsIR, options: ValidationOptions): { valid: boolean; errors: string[]; warnings: string[] } {
    const errors: string[] = [];
    const warnings: string[] = [];
   ...
   * 物理逻辑验证
   */
  private validatePhysics(ir: PhysicsIR, options: ValidationOptions): { valid: boolean; errors: string[]; warnings: string[] } {
    const errors: string[] = [];
    const warnings: string[] = [];
   ...
   * 单位一致性验证
   */
  private validateUnits(ir: PhysicsIR, options: ValidationOptions): { valid: boolean; errors: string[]; warnings: string[] } {
    const errors: string[] = [];
    const warnings: string[] = [];
   ...
   * 数学方程验证
   */
  private validateEquations(ir: PhysicsIR, options: ValidationOptions): { valid: boolean; errors: string[]; warnings: string[] } {
    const errors: string[] = [];
    const warnings: string[] = [];
  ...
  private detectCircularDependencies(ir: PhysicsIR): string[] {
    const circularDeps: string[] = [];
    // 简化的循环依赖检测
    return circularDeps;
  }

  private getExpectedDimension(symbol: string, systemType: string): string | null {
  ...
  private validatePhysicalConstraint(constraint: IRConstraint, ir: PhysicsIR): boolean {
    // 简化的物理约束验证
    return true;
  }
   ...
   * 验证特殊物理量
   */
  private validateSpecialQuantities(param: IRParameter, errors: string[], warnings: string[]): void {
   ...
   * 验证守恒定律
   */
  private validateConservationLaws(ir: PhysicsIR, errors: string[], warnings: string[]): void {
   ...
   * 验证能量守恒
   */
  private validateEnergyConservation(ir: PhysicsIR, law: IRConservationLaw, errors: string[], warnings: string[]): void {
    const energyParams = ir.system.parameters.filter(p => 
      p.symbol.toLowerCase().includes('e') || 
   ...
   * 验证动量守恒
   */
  private validateMomentumConservation(ir: PhysicsIR, law: IRConservationLaw, errors: string[], warnings: string[]): void {
    const momentumParams = ir.system.parameters.filter(p => 
      p.symbol.toLowerCase().includes('p') || 
   ...
   * 验证电荷守恒
   */
  private validateChargeConservation(ir: PhysicsIR, law: IRConservationLaw, errors: string[], warnings: string[]): void {
    const chargeParams = ir.system.parameters.filter(p => 
   ...
   * 验证质量守恒
   */
  private validateMassConservation(ir: PhysicsIR, law: IRConservationLaw, errors: string[], warnings: string[]): void {
    const massParams = ir.system.parameters.filter(p => 
      p.symbol.toLowerCase().includes('m') || 
   ...
   * 验证物理假设的一致性
   */
  private validatePhysicalAssumptions(ir: PhysicsIR, errors: string[], warnings: string[]): void {
    ir.system.modules.forEach(module => {
      // 检查假设与系统类型的一致性
      if (module.assumptions && module.assumptions.includes('小振幅近似') && module.type === 'oscillation') {
        const amplitudeParams = ir.system.parameters.filter(p => 
          p.symbol.toLowerCase().includes('a') || 
      ...
      if (module.assumptions && module.assumptions.includes('经典物理适用')) {
        const velocityParams = ir.system.parameters.filter(p => 
          p.symbol.toLowerCase().includes('v') || 
   ...
   * 验证数值稳定性
   */
  private validateNumericalStability(ir: PhysicsIR, errors: string[], warnings: string[]): void {
    ...
    const hasStiffEquations = ir.system.modules.some(m => 
      m.equations.some(eq => eq.type === 'differential' && eq.order && eq.order > 1)
    );
   ...
   * 计算物理一致性评分
   */
  private calculatePhysicsConsistencyScore(ir: PhysicsIR): number {
    let score = 100;
   ...
   * 推断变量量纲
   */
  private inferVariableDimension(variable: string, expression: string): string {
   ...
   * 推断参数量纲
   */
  private inferParameterDimension(param: string, expression: string): string {
