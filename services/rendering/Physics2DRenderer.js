...
exports.defaultPhysics2DRenderer = exports.Physics2DRenderer = void 0;
const CoordinateSystem_1 = require("./CoordinateSystem");
/**
 * 2D物理渲染器
 */
class Physics2DRenderer {
     ...
     * 渲染单帧物理场景
     */
    async renderFrame(objectStates, environment, config, outputPath) {
     ...
     * 渲染环境（地面、斜面等）
     */
    renderEnvironment(environment, config) {
        // 渲染地面
     ...
     * 渲染物体（确保贴合斜面）
     */
    renderObject(objectState, environment, config) {
        let physicsPosition;
        // 根据物体状态确定物理位置
        if (objectState.phase === '斜面滑动' || objectState.phase === '静止') {
            ...
            if (incline && objectState.inclineDistance !== undefined) {
                // 计算小球在斜面上的精确位置（考虑半径）
                physicsPosition = this.coordinateSystem.calculateInclinePoint(objectState.inclineDistance, incline, config.ballRadius // 物理半径
                );
            }
            else {
        ...
        this.ctx.fillStyle = colors[objectState.phase] || '#FFD93D';
     ...
     * 渲染矢量
     */
    renderVectors(objectState, config) {
        ...
        if (speed > 0.1) {
            ...
            this.drawArrow(this.ctx, screenPosition.x, screenPosition.y, velocityScreenEnd.x, velocityScreenEnd.y, '#FF0000', 3);
        }
        // 重力矢量（始终向下）
        if (objectState.acceleration) {
            ...
            this.drawArrow(this.ctx, screenPosition.x, screenPosition.y, gravityScreenEnd.x, gravityScreenEnd.y, '#0000FF', 2);
        }
    }
    /**
     * 渲染注释
     */
    renderAnnotations(objectStates, environment, config) {
     ...
     * 创建标准斜面定义
     */
    static createStandardIncline(angle, maxDistance, startPoint = { x: 0, y: 0 }, mu_k = 0.2) {
        return {
            angle: angle,
     ...
     * 创建标准环境
     */
    static createStandardEnvironment(inclineAngle, maxDistance, mu_k = 0.2) {
...
exports.Physics2DRenderer = Physics2DRenderer;
/**
 * 默认2D渲染器实例
 */
exports.defaultPhysics2DRenderer = new Physics2DRenderer();
