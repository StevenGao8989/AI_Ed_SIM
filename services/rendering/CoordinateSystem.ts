...
export interface CoordinateConfig {
  scale: number;           // 像素/米 - 全局统一比例
  offsetX: number;         // 屏幕中心X坐标
  offsetY: number;         // 地面Y坐标
  coordinateOrientation: 'xy_y_up' | 'xy_y_down'; // 坐标系方向
}
 ...
 * 斜面定义
 */
export interface InclineDefinition {
  angle: number;           // 角度（度）
  length: number;          // 物理长度（米）
  startPoint: PhysicsPoint; // 起始点
  mu_s?: number;           // 静摩擦系数
  mu_k?: number;           // 动摩擦系数
}
...
export class UnifiedCoordinateSystem {
   ...
   * 屏幕坐标转物理坐标
   */
  screenToWorld(screenPoint: ScreenPoint): PhysicsPoint {
   ...
   * 计算斜面上的点（确保小球贴合斜面）
   */
  calculateInclinePoint(
    distanceAlongIncline: number, 
    incline: InclineDefinition,
   ...
   * 计算斜面的屏幕端点
   */
  calculateInclineScreenPoints(incline: InclineDefinition): { start: ScreenPoint; end: ScreenPoint } {
    const angleRad = incline.angle * Math.PI / 180;
   ...
   * 自动计算合适的斜面长度
   */
  calculateOptimalInclineLength(
    maxDistanceAlongIncline: number, 
    screenWidth: number,
    margin: number = 50
  ): number {
   ...
   * 更新配置
   */
  updateConfig(newConfig: Partial<CoordinateConfig>): void {
 ...
 * 默认坐标系统实例
 */
export const defaultCoordinateSystem = new UnifiedCoordinateSystem();
