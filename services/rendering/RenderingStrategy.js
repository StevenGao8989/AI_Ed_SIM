...
exports.RenderingStrategyFactory = exports.WebGL3DRenderingStrategy = exports.Canvas2DRenderingStrategy = exports.BaseRenderingStrategy = void 0;
exports.ensureGeometryConsistency = ensureGeometryConsistency;
const CoordinateSystem_1 = require("./CoordinateSystem");
/**
 * 基础渲染策略（所有渲染器的基类）
 */
class BaseRenderingStrategy {
    constructor(coordinateConfig) {
        this.coordinateSystem = new CoordinateSystem_1.UnifiedCoordinateSystem(coordinateConfig);
    }
    /**
     * 统一坐标转换（所有子类必须使用此方法）
     */
     ...
     * 斜面位置计算（确保小球贴合斜面）
     */
    calculateInclinePosition(distanceAlongIncline, inclineAngle, objectRadius) {
        const incline = {
            angle: inclineAngle,
            length: distanceAlongIncline * 2,
            startPoint: { x: 0, y: 0 }
        };
        return this.coordinateSystem.calculateInclinePoint(distanceAlongIncline, incline, objectRadius);
    }
    /**
     * 几何一致性验证
     */
    validateGeometry(maxDistance, screenConfig) {
        const incline = {
     ...
     * 自动优化渲染配置
     */
    optimizeRenderConfig(physicsParams, calculationResults, screenConfig) {
        // 使用工厂方法生成最优配置
        const { PhysicsRenderFactory } = require('./PhysicsRenderFactory');
        const recommendation = PhysicsRenderFactory.generateOptimalRenderConfig(PhysicsRenderFactory.analyzePhysicsProblem(physicsParams, calculationResults), screenConfig.width, screenConfig.height);
        // 更新坐标系统
 ...
 * 2D Canvas渲染策略
 */
class Canvas2DRenderingStrategy extends BaseRenderingStrategy {
    /**
     * 渲染斜面（确保与小球轨迹一致）
     */
    renderIncline(ctx, inclineAngle, maxDistance, config) {
        const incline = {
            angle: inclineAngle,
            length: this.coordinateSystem.calculateOptimalInclineLength(maxDistance, config.width, 50),
            startPoint: { x: 0, y: 0 }
        };
     ...
     * 渲染物体（确保在斜面上时贴合斜面）
     */
    renderObject(ctx, objectState, environment, config) {
        let physicsPosition;
        if (objectState.phase === '斜面滑动' || objectState.phase === '静止') {
            // 斜面运动：使用精确的斜面位置计算
            physicsPosition = this.calculateInclinePosition(objectState.inclineDistance || objectState.position.x, environment.theta || 30, config.ballRadius || 0.1);
        }
        else {
            // 其他阶段：直接使用物体位置
        ...
        ctx.fillStyle = colors[objectState.phase] || '#FFD93D';
        ctx.strokeStyle = '#000';
        ctx.lineWidth = 2;
        ctx.beginPath();
...
exports.Canvas2DRenderingStrategy = Canvas2DRenderingStrategy;
/**
 * 3D WebGL渲染策略
 */
class WebGL3DRenderingStrategy extends BaseRenderingStrategy {
}
exports.WebGL3DRenderingStrategy = WebGL3DRenderingStrategy;
/**
 * 渲染策略工厂
 */
class RenderingStrategyFactory {
     ...
     * 创建渲染策略
     */
    static createStrategy(type, physicsParams, calculationResults, screenConfig) {
        switch (type) {
            case '2d_canvas':
...
exports.RenderingStrategyFactory = RenderingStrategyFactory;
/**
 * 几何一致性保证装饰器
 */
function ensureGeometryConsistency(target, propertyKey, descriptor) {
